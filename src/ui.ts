...
import {
  voltearLaCarta,
  superadosLosIntentos,
  cambiarEstadoAPartidaCompleta,
...
const botonEmpezarPartida = () => {
  ...
  if (botonIniciar && botonIniciar instanceof HTMLButtonElement) {
    ...
    console.error("No existe el boton");
  }
};
...
const pintarCartas = () => {
  for (let i = 0; i < tablero.cartas.length; i++) {
    ...
    if (
      elementContenidoJuego !== null &&
      elementContenidoJuego !== undefined &&
      elementContenidoJuego instanceof HTMLDivElement
    ) {
...
const reiniciarCartas = () => {
  const divContenidoJuego = document.getElementById("contenidoJuego");

  if (
    divContenidoJuego !== null &&
    divContenidoJuego !== undefined &&
    divContenidoJuego instanceof HTMLDivElement
  ) {
    divContenidoJuego.innerHTML = "";
    pintarCartas();
    intentosJuego.numeroDeIntentos = 0;
  } else {
    console.error("No hay div con id contenidoJuego");
  }
};
...
const pintarIntentos = () => {
  ...
  if (
    intentosElement !== null &&
    intentosElement !== undefined &&
    intentosElement instanceof HTMLDivElement
  ) {
    intentosElement.innerHTML = `N° de intentos realizados:  ${intentosJuego.numeroDeIntentos} de 10`;
  } else {
    console.error("No hay elemento con id intentos");
  }
};
...
const pintarMensajes = (mensaje: string) => {
  ...
  if (
    mensajesPartidaElement !== null &&
    mensajesPartidaElement !== undefined &&
    mensajesPartidaElement instanceof HTMLDivElement
  ) {
    mensajesPartidaElement.textContent = mensaje;
  } else {
    console.error("No hay elemento con id mensajesPartida");
  }
};
...
const partidaTerminada = () => {
  if (superadosLosIntentos()) {
    ...
    const mensaje = "❌ - Has superado el numero maximo de intentos - ❌";
    pintarMensajes(mensaje);
    juegoBloqueado();
  }
};
...
const juegoBloqueado = () => {
  ...
  if (
    elementJuego !== null &&
    elementJuego !== undefined &&
    elementJuego instanceof HTMLDivElement
  ) {
...
const noSePuedeVoltearLaCarta = (idElemento: string) => {
  if (idElemento.includes("imagen")) {
    const mensaje = "⚠️ - Cuidado: la carta elegida ya está volteada - ⚠️";
    pintarMensajes(mensaje);
  }
};
  ...
  if (sePuedeVoltearLaCarta(tablero, Number(idElemento))) {
    ...
    if (!isNaN(indiceImagen)) {
      ...
      if (carta && carta.estaVuelta) {
        ...
        if (
          elementImagen !== null &&
          elementImagen !== undefined &&
          elementImagen instanceof HTMLImageElement
        ) {
...
const ocultarElemento = (indice: number) => {
  ...
  if (
    ...
    console.error(
      "El elemento con id `imagen_${indice}` no es un elemento HTMLImageElement"
    );
  }
};
